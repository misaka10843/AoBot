...
whitelist = getattr(config, "analysis_whitelist", [])
group_whitelist = getattr(config, "analysis_group_whitelist", [])
...
async def analysis_main(event: Event) -> None:
    ...
    if not whitelist:
      if blacklist and int(event.get_user_id()) in blacklist:
          return
    elif whitelist and int(event.get_user_id()) not in whitelist:
      return
    ...
    if not group_whitelist:
      if group_id in group_blacklist:
          return
    elif group_id not in group_whitelist:
      return
